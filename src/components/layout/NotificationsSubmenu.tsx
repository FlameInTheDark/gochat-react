...
export function NotificationsDropdownSubmenu({ current, onUpdate }: Props) {
  const { t } = useTranslation()
  const level = current?.notifications ?? ModelNotificationsType.NotificationsAll
    ...
    <>
      ...
      <DropdownMenuSub>
        ...
        <DropdownMenuSubContent>
          ...
          <DropdownMenuItem onClick={() => onUpdate({ muted: true, muted_until: muteUntilIso(15) })} onSelect={(e) => e.preventDefault()}>
            {t('notifications.muteFor15min')}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onUpdate({ muted: true, muted_until: muteUntilIso(60) })} onSelect={(e) => e.preventDefault()}>
            {t('notifications.muteFor1hour')}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onUpdate({ muted: true, muted_until: muteUntilIso(480) })} onSelect={(e) => e.preventDefault()}>
            {t('notifications.muteFor8hours')}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onUpdate({ muted: true, muted_until: muteUntilIso(1440) })} onSelect={(e) => e.preventDefault()}>
            {t('notifications.muteFor24hours')}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => onUpdate({ muted: true, muted_until: undefined })} onSelect={(e) => e.preventDefault()}>
            {t('notifications.muteIndefinite')}
          </DropdownMenuItem>
        </DropdownMenuSubContent>
      </DropdownMenuSub>
